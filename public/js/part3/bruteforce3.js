const char3 ="0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!@#$%^&*()_+[]{}|;:',./<>?";


function bruteForceAttackAll(targetPassword) {
  const maxIndex = char3.length;
  let currentPassword = "";
  const startTime = new Date();

  function generateNextPassword(password) {
    if (password === "") {
      return char3[0];
    }

    let i = password.length - 1;
    let carry = true;

    while (i >= 0 && carry) {
      const charIndex = char3.indexOf(password[i]);
      if (charIndex < maxIndex - 1) {
        password =
          password.substring(0, i) +
          char3[charIndex + 1] +
          password.substring(i + 1);
        carry = false;
      } else {
        password =
          password.substring(0, i) + char3[0] + password.substring(i + 1);
        i--;
      }
    }

    if (carry) {
      password = char3[0] + password;
    }

    return password;
  }

  while (currentPassword !== targetPassword) {
    currentPassword = generateNextPassword(currentPassword);
    console.log(
      "Testing password:",
      currentPassword,
      "Length:",
      currentPassword.length
    );
  }

  const endTime = new Date();
  const elapsedTime = (endTime - startTime) / 1000; // in seconds
  document.getElementById("foundPassword").textContent = currentPassword;
  document.getElementById("elapsedTime").textContent = elapsedTime;
  return {
    password: currentPassword,
    startTime,
    endTime,
    elapsedTime,
  };
}

function startBruteForceAttackAll() {
  const targetPassword = document.getElementById("passwordInput3").value;

  if (targetPassword === "") {
    alert("Veuillez entrer un mot de passe.");
    return;
  }

  bruteForceAttackAll(targetPassword);
}